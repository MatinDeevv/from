#!/usr/bin/env python3
"""Fetch Dukascopy XAUUSD ticks and aggregate to 5-minute bars."""
...
def fetch_hour(hour_ms: int) -> tuple[int, bytes | None, str | None]:
    url = archive_url(hour_ms)
    for attempt in range(4):
        try:
            ...
            text = repr(exc)
            if "HTTPError 404" in text:
                return hour_ms, None, "404"
            if attempt < 3:
                time.sleep(0.5 * (attempt + 1))
                continue
            return hour_ms, None, text
    return hour_ms, None, "unknown"


def aggregate_payload(hour_ms: int, payload: bytes, start_ms: int, end_ms: int) -> pd.DataFrame:
    ...
    ask = records["ask"][keep].astype(np.float64) / 1000.0
    bid = records["bid"][keep].astype(np.float64) / 1000.0
...
def main() -> int:
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=args.workers) as ex:
        futures = {ex.submit(fetch_hour, hour): hour for hour in hours}
        for i, fut in enumerate(concurrent.futures.as_completed(futures), 1):
            hour_ms, payload, err = fut.result()
            if payload is not None:
                frame = aggregate_payload(hour_ms, payload, start_ms, end_ms)
                if not frame.empty:
                    frames.append(frame)
